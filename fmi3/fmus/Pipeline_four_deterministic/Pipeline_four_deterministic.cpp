...
#include "Pipeline_four_deterministic.h"
#include <ostream>
#include <iostream>
#include <limits>
#include <algorithm>
...
Pipeline_four_deterministic::Pipeline_four_deterministic(
    ...
    eventHappenedInternal3(fmi3False),
    eventHappenedInternal4(fmi3False),
    in1_( 0 ),
    in2_( 0 ),
    out1_( 0 ),
    out2_( 0 ),
...
{
    std::cout << ">> Pipeline_four_deterministic::constructor" << std::endl << std::flush;
    ...
    this->logDebug(
        "successfully initialized class %s", "Pipeline_four_deterministic"
    );
    std::cout << "<< Pipeline_four_deterministic::constructor" << std::endl << std::flush;
}

fmi3Status Pipeline_four_deterministic::enterInitializationMode(
    ...
    std::cout << ">> Pipeline_four_deterministic::enterInitializationMode" << std::endl << std::flush;
    ...
    if ( fmi3True == toleranceDefined )
    {
        ...
        pipe3.setTolerance(tolerance);
        pipe4.setTolerance(tolerance);
...
fmi3Status
Pipeline_four_deterministic::exitInitializationMode()
{
    std::cout << ">> Pipeline_four_deterministic::exitInitializationMode" << std::endl << std::flush;
 ...
 */
fmi3Status 
Pipeline_four_deterministic::enterEventMode(
    ...
    std::cout << ">> Pipeline_four_deterministic::enterEventMode" << std::endl << std::flush;
    ...
    std::cout << "  eventHappenedInternal1=" << this->eventHappenedInternal1 << ", eventHappenedInternal2=" << this->eventHappenedInternal2 << ", eventHappenedInternal3=" << this->eventHappenedInternal3 << ", eventHappenedInternal4=" << this->eventHappenedInternal4 << std::endl << std::flush;
    ...
    if ( fmi3True == this->eventHappenedInternal3 ) {
        pipe3.processInternalEvent();
    }
    if ( fmi3True == this->eventHappenedInternal4 ) {
        pipe4.processInternalEvent();
    }
    if (ic23.hasMessage()) {
        out1_ = ic23.getMessage();
        outClock1_ = fmi3ClockActive;
    }
    if (ic34.hasMessage()) {
        out2_ = ic34.getMessage();
        outClock2_ = fmi3ClockActive;
    }
    std::cout << "<< Pipeline_four_deterministic::!enterEventMode" << std::endl << std::flush;
    return fmi3OK;
}
...
fmi3Status
Pipeline_four_deterministic::reset()
{
    std::cout << ">> Pipeline_four_deterministic::reset" << std::endl << std::flush;
    pipe1.reset();
    pipe2.reset();
    pipe3.reset();
    pipe4.reset();
...
fmi3Status
Pipeline_four_deterministic::getInt32(
    const fmi3ValueReference valueReferences[],
    size_t nValueReferences,
    fmi3Int32 values[],
    size_t nValues
) {
    std::cout << ">> Pipeline_four_deterministic::getInt32" << std::endl << std::flush;
    ...
    for (
        ...
        switch ( *vr ) {
            case this->vrOut1_:
                *v = this->out1_;
                this->logDebug( "%d => get1 %d", *vr, this->out1_ );
                break;
            case this->vrOut2_:
                *v = this->out2_;
                this->logDebug( "%d => get2 %d", *vr, this->out2_ );
                break;
            default:
...
fmi3Status
Pipeline_four_deterministic::getClock(
    const fmi3ValueReference valueReferences[],
    size_t nValueReferences,
    fmi3Clock values[]
) {
    std::cout << ">> Pipeline_four_deterministic::getClock" << std::endl << std::flush;
    /*if ( nValueReferences != nValues ) {
        this->logError(
            "%s %s",
            "This FMU only supports scalar variables.",
    ...
    for (
        ...
        switch ( *vr ) {
            case this->vrOutClock1_:
                *v = this->outClock1_;
                this->logDebug( "%d => get clock1 %d", *vr, this->outClock1_ );
                break;
            case this->vrOutClock2_:
                *v = this->outClock2_;
                this->logDebug( "%d => get clock2 %d", *vr, this->outClock2_ );
                break;
            default:
                this->logError( "Invalid value reference: %d", *vr );
                status = fmi3Error;
...
fmi3Status
Pipeline_four_deterministic::setInt32(
    const fmi3ValueReference valueReferences[],
    size_t nValueReferences,
    const fmi3Int32 values[],
    size_t nValues
) {
    std::cout << ">> Pipeline_four_deterministic::setInt32" << std::endl << std::flush;
    if ( nValueReferences != nValues ) {
        this->logError(
            "%s %s",
            "This FMU only supports scalar variables.",
    ...
    for (
        ...
        switch ( *vr ) {
            ...
            case this->vrRandomSeed_:
                ...
                pipe3.setRandomSeed(*v);
                pipe4.setRandomSeed(*v);
                break;
            default:
                this->logError( "Invalid value reference: %d", *vr );
                status = fmi3Error;
...
fmi3Status
Pipeline_four_deterministic::setFloat64(
    const fmi3ValueReference valueReferences[],
    size_t nValueReferences,
    const fmi3Float64 values[],
    size_t nValues
) {
    std::cout << ">> Pipeline_four_deterministic::setFloat64" << std::endl << std::flush;
    if ( nValueReferences != nValues ) {
        this->logError(
            "%s %s",
            "This FMU only supports scalar variables.",
...
fmi3Status
Pipeline_four_deterministic::setClock(
    const fmi3ValueReference valueReferences[],
    size_t nValueReferences,
    const fmi3Clock values[])
{
    std::cout << ">> Pipeline_four_deterministic::setClock" << std::endl << std::flush;
/*    if ( nValueReferences != nValues ) {
        this->logError(
            "%s %s",
            "This FMU only supports scalar variables.",
    ...
    for (
        ...
        switch ( *vr ) {
            case this->vrInClock1_:
                ...
                this->inClock1_ = *v;
                this->logDebug( "%d => set clock %d", *vr, this->inClock1_ );
                break;
            case this->vrInClock2_:
                if ( fmi3ClockInactive == *v )
                {
                    this->logError(
                        "clocks may not be deactivated by the importer"
                ...
                this->inClock2_ = *v;
                this->logDebug( "%d => set clock %d", *vr, this->inClock2_ );
                break;
            default:
                this->logError( "Invalid value reference: %d", *vr );
                status = fmi3Error;
...
fmi3Status
Pipeline_four_deterministic::updateDiscreteStates(
    ...
    std::cout << ">> Pipeline_four_deterministic::updateDiscreteStates" << std::endl << std::flush;
    // Input clock is active --> add message as output using the calculated delay.
    if ( fmi3ClockActive == this->inClock1_ ) {
        pipe1.insertMessage(syncTime_,in1_,&ic12/*&out_,&outClock_*/);
    }
    if ( fmi3ClockActive == this->inClock2_ ) {
        pipe2.insertMessage(syncTime_,in2_,&ic12/*&out_,&outClock_*/);
    }
    if (ic12.hasMessage()) {
        fmi3Int32 msg=ic12.getMessage();
        std::cout << "### Message from pipe1/2: " << msg << ", inserting into pipe3+4" << std::endl << std::flush;
        pipe3.insertMessage(syncTime_,msg,&ic23);
        pipe4.insertMessage(syncTime_,msg,&ic34);
    }
    ...
    *nextEventTimeDefined = fmi3False;
    ...
    fmi3Float64 net3=pipe3.processQueue(syncTime_, nextEventTimeDefined);
    netime=std::min(netime,net3);
    fmi3Float64 net4=pipe4.processQueue(syncTime_, nextEventTimeDefined);
    netime=std::min(netime,net4);
    std::cout << "%%%%%%% Next Event times: pipe 1=" << net1 << ", pipe2=" << net2 << ", pipe3=" << net3 << ", pipe4=" << net4 << ", combined=" << netime << std::endl << std::flush;
...
fmi3Status
Pipeline_four_deterministic::doStep(
    ...
    std::cout << ">> Pipeline_four_deterministic::doStep" << std::endl << std::flush;
    ...
    eventHappenedInternal3 = fmi3False; //Default
    eventHappenedInternal4 = fmi3False; //Default
    ...
    else if (pipe3.didWeOverstep(syncTime_)) {
        fmi3Float64 next=pipe3.getNextEventTime();
        std::cout << "xxd3" << std::endl;
        this->logDebug(
            "%s %s %f",
            "The importer stepped over an event in pipe 3.",
            "The current internal time (lastSuccessfulTime) is: ",next);
        *eventEncountered = fmi3True;
        eventHappenedInternal3 = fmi3True;
        *earlyReturn = fmi3True;
        *lastSuccessfulTime = next;
    }
    else if (pipe4.didWeOverstep(syncTime_)) {
        fmi3Float64 next=pipe4.getNextEventTime();
        std::cout << "xxd4" << std::endl;
        this->logDebug(
            "%s %s %f",
            "The importer stepped over an event in pipe 4.",
            "The current internal time (lastSuccessfulTime) is: ",next);
        *eventEncountered = fmi3True;
        eventHappenedInternal4 = fmi3True;
        *earlyReturn = fmi3True;
        *lastSuccessfulTime = next;
    }
    ...
    else if (pipe3.didWeReach(syncTime_)) {
        std::cout << "xxe3" << std::endl;
        this->logDebug(
            "The importer has reached the next pipe3 event at the new synchronization point."
        );
        if (pipe3.isQueueCorrupt()) {
            this->logError("corrupted event queue in pipe3");
            *terminateSimulation = fmi3True;
            return fmi3Fatal;
        }
        *eventEncountered = fmi3True;
    ...
    else if (pipe4.didWeReach(syncTime_)) {
        std::cout << "xxe4" << std::endl;
        this->logDebug(
            "The importer has reached the next pipe4 event at the new synchronization point."
        );
        if (pipe4.isQueueCorrupt()) {
            this->logError("corrupted event queue in pipe4");
            *terminateSimulation = fmi3True;
            return fmi3Fatal;
        }
        *eventEncountered = fmi3True;
...
void
Pipeline_four_deterministic::deactivateAllClocks()
{
    std::cout << ">> Pipeline_four_deterministic::deactivateAllClocks" << std::endl << std::flush;
    inClock1_ = fmi3ClockInactive;
    inClock2_ = fmi3ClockInactive;
    outClock1_ = fmi3ClockInactive;
    outClock2_ = fmi3ClockInactive;
    std::cout << "<< Pipeline_four_deterministic::deactivateAllClocks" << std::endl << std::flush;
}
