...
def testFMU4(model_path, model_name, instance_name, visible, logging_on, event_mode, early_return):
    with TemporaryDirectory() as tmpdir:
        print("path=",os.path.join( model_path, model_name + '.fmu' ))
        extractFMU(
    ...
    val_in1 = [ 0 ]
    val_in_clock1 = [ fmu.fmi3_clock_active ]
    val_in2 = [ 0 ]
    val_in_clock2 = [ fmu.fmi3_clock_active ]
    val_out1 = [ 0 ]
    val_out_clock1 = [ fmu.fmi3_false ];
    val_out2 = [ 0 ]
    val_out_clock2 = [ fmu.fmi3_false ];
    ...
    while ( time < stop_time ):
        ...
        if not step_result.event_encountered:
            ...
            print("At time "+str(time)+": SEND message with ID = "+str(msg_id)+" on pipe "+str(channel), flush=True)
            ...
            if alternate:
                val_in2[0] = msg_id
                status = fmu.fmi3SetInt32(['in2'],val_in2)
                val_in_clock2[0] = fmu.fmi3_clock_active
                status = fmu.fmi3SetClock(['inClock2'],val_in_clock2)
                alternate=False
            else:
                val_in1[0] = msg_id
                status = fmu.fmi3SetInt32(['in1'],val_in1)
                val_in_clock1[0] = fmu.fmi3_clock_active
                status = fmu.fmi3SetClock(['inClock1'],val_in_clock1)
                alternate=True
        ...
        # Internal event: message has arrived at output.
            ...
            val_out_clock1 = fmu.fmi3GetClock(['outClock1'])
            val_out_clock2 = fmu.fmi3GetClock(['outClock2'])
            got_message=False

            if val_out_clock1[0] == fmu.fmi3_clock_active:
                val_out1 = fmu.fmi3GetInt32(['out1'])
                got_message=True
                print("At time "+str(time)+": RECEIVE message from channel 1 with ID = "+str(val_out1[0]), flush=True)
            if val_out_clock2[0] == fmu.fmi3_clock_active:
                val_out2 = fmu.fmi3GetInt32(['out2'])
                got_message=True
                print("At time "+str(time)+": RECEIVE message from channel 2 with ID = "+str(val_out2[0]), flush=True)
            if not got_message:
